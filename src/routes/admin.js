...
// ─── GET /api/admin/requests ─── Media requests queue ────────────────────
...
// ─── PATCH /api/admin/requests/:id/flag ─── Flag a request ───────────────
...
router.post('/media', requireAdmin, [
  ...
  body('imageUrl').optional({ nullable: true }).isURL().withMessage('Must be a valid URL'),
  ...
  try {
    const {
      ...
      // Person names (we create/find them automatically)
      directorNames, castNames, authorNames, designerNames,
    } = req.body;

    const slug = await uniqueSlug(slugify(title, releaseYear));

    // Upsert persons by name
    const connectPersons = async (names) => {
      if (!names?.length) return undefined;
      const persons = await Promise.all(names.map(name => {
        const personSlug = name.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
    ...
    const item = await prisma.mediaItem.create({
      data: {
        ...
        genres: genres || [],
        imdbId, goodreadsId, openCriticId,
        studio, runtime: runtime ? parseInt(runtime) : null,
        ...
        designers: await connectPersons(designerNames),
      },
      include: {
        directors: true, cast: true, authors: true, designers: true,
      },
    });

    // Kick off external ratings fetch
...
// ─── PATCH /api/admin/media/:id ─── Edit a media item ────────────────────
router.patch('/media/:id', requireAdmin, async (req, res, next) => {
  try {
    const allowed = [
      ...
      'metacriticScore', 'studio', 'runtime', 'seasons', 'episodes',
      'streamingOn', 'mpaaRating', 'isbn', 'publisher', 'pageCount',
      'publishers', 'platforms', 'minPlayers', 'maxPlayers', 'playTimeMinutes',
    ];
